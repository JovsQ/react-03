...
class BluetoothSerialExample extends Component {
	...
	componentWillMount() {
		...
		BluetoothSerial.on('bluetoothDisableed', () => Alert.alert('Bluetooth disabled'));
	...
	pairDevice(device) {
		...
		.then((paired) => {
			if (paired) {
				Alert.alert(`Device ${device.name} pairing successfully`);
	...
	write(message) {
		if (!this.state.connected) {
			Alert.alert('You must connect to a device first');
		} else {
